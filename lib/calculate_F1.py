...
def _F1_core(centers_DB : np.ndarray, boxes : np.ndarray, score : np.ndarray, det_thres:float, detclass:float):

        to_keep = score>det_thres
        boxes = boxes[to_keep]
        
        to_keep=boxes[:,4]==detclass
        boxes = boxes[to_keep]
...
def calculate_F1(databasefile, result_boxes=None, resfile=None, det_thres=0.5, detclass=0, hotclass=2,verbose=False):
    ...
    print('Official count of relevant cells in DB: ', mitcount)
...
#            print('ID:',resfile,DB.findSlideWithFilename(resfile,''))
            ...
            F1,TP,FP,FN = _F1_core(centers_DB, boxes, score,det_thres, detclass=detclass)
    ...
    print('Overall: ')
    sF1 = 2*sTP/(2*sTP + sFP + sFN+0.00001)
    print('TP:', sTP, 'FP:', sFP,'FN: ',sFN,'F1:',sF1)
    print('Number of all cells:',sP)
    print('Precision: %.3f '%(sTP / (sTP+sFP+0.00001)))
    print('Recall: %.3f' %(sTP / (sTP+sFN+0.000001)))
...
def optimize_threshold(databasefile, result_boxes=None, resfile=None, detclass=0, hotclass=2, minthres=0.5):
